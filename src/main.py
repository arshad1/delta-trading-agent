...
def main():
    ...
    clear_terminal()
    parser = argparse.ArgumentParser(description="LLM-based Trading Agent on Delta Exchange")
    ...
    exchange = DeltaExchangeAPI()
    agent = TradingAgent(hyperliquid=exchange)
    ...
    async def run_loop():
        ...
        # Pre-load product metadata and set leverage for all traded assets
        await exchange.init_products(args.assets)
        add_event(f"Delta Exchange products initialised for: {args.assets}")
        ...
        while True:
            ...
            try:
                ...
                for ptc in positions_to_close:
                    ...
                    try:
                        if is_long:
                            await exchange.place_sell_order(coin, size)
                        else:
                            await exchange.place_buy_order(coin, size)
            ...
            recent_fills_struct = []
            try:
                fills = await exchange.get_recent_fills(limit=50)
            ...
            for asset in args.assets:
                try:
                    current_price = await exchange.get_current_price(asset)
                    ...
                    oi = await exchange.get_open_interest(asset)
                    funding = await exchange.get_funding_rate(asset)
            ...
            try:
                outputs = await agent.decide_trade(args.assets, context)
            ...
            if _is_failed_outputs(outputs):
                ...
                try:
                    outputs = await agent.decide_trade(args.assets, context_retry)
                    if not isinstance(outputs, dict):
                ...
                try:
                    ...
                    if action in ("buy", "sell"):
                        ...
                        if order_type == "limit" and limit_price:
                            ...
                            if is_buy:
                                order = await exchange.place_limit_buy(asset, alloc_usd, limit_price)
                            else:
                                order = await exchange.place_limit_sell(asset, alloc_usd, limit_price)
                            add_event(f"LIMIT {action.upper()} {asset} ~${alloc_usd} at limit ${limit_price}")
                        else:
                            if is_buy:
                                order = await exchange.place_buy_order(asset, alloc_usd)
                            else:
                                order = await exchange.place_sell_order(asset, alloc_usd)

                        # Confirm by checking recent fills for this asset shortly after placing
                        await asyncio.sleep(1)
                        fills_check = await exchange.get_recent_fills(limit=10)
                        filled = False
    ...
    async def check_exit_condition(trade, exchange_api):
        """Evaluate whether a given trade's exit plan triggers a close."""
        plan = (trade.get("exit_plan") or "").lower()
        if not plan:
            return False
        try:
            candles_4h = await exchange_api.get_candles(trade["asset"], "4h", 60)
            ...
            if "close above ema50" in plan:
                ...
                current = await exchange_api.get_current_price(trade["asset"])
